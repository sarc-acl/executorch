...
/*
 * specs/023-8da4w-int8-dbuf-sweep: dbuf2 ("store-first") loop-structure
 * port, selected via ET_VK_DQ8CA_COOPMAT_VARIANT=dbuf2. Re-derived from the
 * shipped dbuf4 shader (linear_dq8ca_qw_coopmat.glsl) per
 * shmem_double_buf2.comp: the prologue only prefetches chunk 0 into temp
 * registers (no shared-memory write, no barrier); each loop iteration then
 * does store(temp -> cur slice) -> barrier() [UNCONDITIONAL, every
 * iteration] -> MMA(cur) -> prefetch(next chunk -> temp) [skipped on the
 * last chunk]. This inverts which iteration boundary "owns" the store: in
 * dbuf4, iteration `chunk` stores the data for `chunk+1` into the next
 * slice; here, iteration `chunk` stores the data FOR ITSELF (already
 * prefetched by the previous iteration, or by the prologue for chunk 0)
 * into the current slice, immediately before using it. The same inversion
 * applies to the group wsum/wsc ping-pong: instead of storing the NEXT
 * group's values at the tail of the current group's last chunk, this
 * variant stores the CURRENT group's values (prefetched by the previous
 * group's last chunk) at the head of the group's first chunk. Group 0's
 * wsum/wsc are unaffected -- they are set up directly in the prologue in
 * every variant, exactly as in dbuf4. All other int8-specific logic (weight
 * unpack, Xclipse spec-constant workarounds) is unchanged from dbuf4. The
 * nested groups x chunks loop and unconditional group epilog are kept
 * exactly as in dbuf4 -- flattening them is what crashes the Xclipse PAL
 * compiler (see below), not what this variant is testing.
 *
