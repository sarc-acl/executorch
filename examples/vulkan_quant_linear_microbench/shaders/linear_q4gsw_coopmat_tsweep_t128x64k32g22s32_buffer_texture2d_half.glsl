...
/*
 * TILE/SUBGROUP-SWEEP variant of linear_qw_coopmat.glsl's INT4 (q4gsw) path
 * (specs/028-4w-e2e-tile-sweep). This file is a structural copy of
 * linear_qw_coopmat.glsl's WEIGHT_INT4 branch -- same dbuf1 (prefetch-first
 * double-buffer) loop, same dequant_block, same bindings/spec-constants --
 * with the INT8 branch removed (this shader is q4gsw-only) and the tile
 * geometry (WG_TILE_*, SG_GRID_*, SUBGROUP_SIZE) exposed as yaml-level
 * per-variant parameters, selected at dispatch via
 * ET_VK_Q4GSW_COOPMAT_VARIANT=tsweep_t<M>x<N>k<K>g<SGX><SGY>s<sub>
 * (QuantizedLinear.cpp), mirroring specs/023/025/026's
 * ET_VK_DQ8CA_COOPMAT_VARIANT mechanism for the int8 dq8ca_q4gsw shader.
 *
