...
def score_candidate(cfg: dict) -> tuple[float, float, float]:
    ...
    register_penalty = (
        1
        + max(0, cfg["accumulators_per_sg"] - REGISTER_PENALTY_THRESHOLD)
        * REGISTER_PENALTY_SLOPE
    )
...
def main():
    ...
    if args.known_measurements:
        anchors, excluded_known = load_known_measurement_anchors(
            args.known_measurements
        )
    ...
    missing_anchors = [
        t for t in anchors if t not in {r["candidate_token"] for r in scored}
    ]
    if missing_anchors:
