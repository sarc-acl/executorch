...
GROUP_SIZE = 128
LDS_LIMIT_BYTES = (
    65536  # confirmed via test_coopmat_probe on M5 EVT1: maxComputeSharedMemorySize
)
WG_INVOCATION_LIMIT = (
    1024  # confirmed via test_coopmat_probe: maxComputeWorkGroupInvocations
)
FP16_PER_VEC4 = 8
...
class TileConfig:
    ...
    def is_valid(self) -> bool:
        ...
        if a_rows_per_pass == 0 or b_rows_per_pass == 0:
            return False
        if (
            self.wg_tile_m % a_rows_per_pass != 0
            or self.wg_tile_k % b_rows_per_pass != 0
        ):
            return False
