...
def precheck(ssh_host, serial):
    ...
    _, out2, _ = ssh_run(
        ssh_host,
        f'adb -s {serial} shell "ps -A | grep -iE \\"llama|coopmat\\"" || true',
    )
...
def cmd_round(args, round_name, candidates, run_count_per_candidate, quick=False):
    ...
    if not ok:
        out_path.write_text(
            json.dumps([{"halted": True, "halt_reason": precheck_info}], indent=2)
        )
    ...
    for i, token in enumerate(candidates, start=1):
        ...
        result = {
            ...
            "tiled_gflops_ref": (
                round(mean_stddev(tiled_samples)[0], 2) if tiled_samples else 0.0
            ),
        ...
        budget["configs_measured_on_hardware"] = len(
            {_.get("candidate_token") for _ in _all_measured(args.out_dir, results)}
        )
...
def main():
    ...
    if args.round == "1":
        ...
        candidates = args.tokens or [
            r["candidate_token"] for r in survivors[:top_third_n]
        ]
