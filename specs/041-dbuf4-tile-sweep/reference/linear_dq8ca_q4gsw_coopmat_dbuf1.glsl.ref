...
/*
 * specs/023-8da4w-int8-dbuf-sweep: dbuf1 ("prefetch-first") loop-structure
 * port, selected via ET_VK_DQ8CA_COOPMAT_VARIANT=dbuf1. Re-derived from the
 * shipped dbuf4 shader (linear_dq8ca_qw_coopmat.glsl) per
 * shmem_double_buf.comp: the prologue's chunk-0 shared-memory write gets its
 * own explicit barrier (instead of relying on the main loop's first
 * iteration to barrier it), and each loop iteration's barrier moves from the
 * TOP (before that iteration's prefetch) to the BOTTOM (right after that
 * iteration's store into the next slice), matching the reference's
 * per-iteration order: prefetch(next) -> MMA(cur) -> store(next) -> barrier.
 * This is functionally the same barrier-vs-data ordering as dbuf4 in the
 * loop's steady state ("barrier at the end of iteration N" executes at the
 * exact point as "barrier at the start of iteration N+1") -- the only real
 * difference is which boundary (prologue vs. first-iteration) owns the
 * barrier that makes chunk 0's slice visible, and that this variant skips
 * its trailing barrier entirely on the very last chunk (nothing needs to be
 * made visible after it) rather than dbuf4's explicit peeled epilogue. All
 * int8-specific logic (weight unpack, group wsum/wsc ping-pong, Xclipse
 * spec-constant workarounds) is unchanged from dbuf4. The nested groups x
 * chunks loop and unconditional group epilog are kept exactly as in dbuf4 --
 * flattening them is what crashes the Xclipse PAL compiler (see below), not
...
void main() {
  ...
  // dbuf1: prefetch chunk 0 into temp registers, then store to slice 0, then
  // barrier here (below) -- unlike dbuf4, this variant does NOT rely on the
  // main loop's first iteration to barrier it; the loop's own barrier moves
  // to the end of each iteration instead (see MAIN LOOP below).
  if (a_active) {
  ...
  // spec-resolved trip counts). One barrier per chunk, at the END of the
  // iteration instead of the start (dbuf1 "prefetch-first" ordering). Chunk
  // iteration (global index `chunk`):
  //   1. prefetch  — chunk+1 (A blocks, B blocks) into temp; when chunk+1
  ...
  //   4. barrier   — makes slice ((chunk+1)%2) (and, on a group boundary,
  //                  wsum/wsc slice ((g+1)%2)) visible for the next
  //                  iteration. Skipped on the final chunk (nothing further
  ...
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    for (uint inner = 0; inner < CHUNKS_PER_GROUP; ++inner, ++chunk) {
      ...
      if (has_next) {
        ...
        // --- 4. barrier: nxt slice(s) now fully written, make visible to
        //     the next iteration (skipped when there is no next iteration).
        barrier();
      }
    }  // chunks
