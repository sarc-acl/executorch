...
/*
 * specs/023-8da4w-int8-dbuf-sweep: dbuf3 (dbuf2 with the true final chunk
 * peeled out of the loop) loop-structure port, selected via
 * ET_VK_DQ8CA_COOPMAT_VARIANT=dbuf3. Re-derived from the dbuf2 port
 * (linear_dq8ca_q4gsw_coopmat_dbuf2.glsl) per shmem_double_buf3.comp: same
 * store(cur) -> barrier() -> MMA(cur) -> prefetch(next) per-chunk ordering
 * as dbuf2, but the nested groups x chunks loop runs one FEWER chunk overall
 * (the last group's inner-loop trip count is reduced by one), so its
 * prefetch step is UNCONDITIONAL inside the loop body (no `has_next` check)
 * -- the very last chunk (last chunk of the last group) is instead handled
 * by an explicit, unconditional epilogue block (store -> barrier -> MMA, no
 * prefetch) placed right before that group's existing dequant epilog. The
 * only branch this adds beyond dbuf2 is a per-*group* (not per-chunk) check
 * of whether the current group is the last one, evaluated `num_groups`
 * times total (small), not once per chunk -- a materially different risk
 * profile than the per-chunk "conditional coopmat epilog at large
 * spec-resolved trip counts" that crashes the Xclipse PAL compiler (see
 * below); this variant does not flatten the groups x chunks nesting itself.
 * The group wsum/wsc ping-pong inversion is unchanged from dbuf2. All other
 * int8-specific logic (weight unpack, Xclipse spec-constant workarounds) is
 * unchanged from dbuf4.
 *
...
void main() {
  ...
  // iteration -- this loop runs one FEWER chunk than dbuf2 overall (the last
  // group's trip count is reduced by one via chunks_this_group below), so
  // its own prefetch step never needs a `has_next` guard; the true final
  // chunk is handled by an explicit peeled epilogue after this loop instead
  // (dbuf3 "no conditionals in the loop body" ordering). Chunk iteration
  // (global index `chunk`):
  ...
  //                  slice (group_i%2)) fully written.
  //   3. int8 MMA  — on slice (chunk%2) into accum_int32.
  //   4. prefetch  — chunk+1 (A blocks, B blocks) into temp, UNCONDITIONAL;
  //                  when chunk+1 starts a new group, also that group's
  //                  wsum/wsc element.
  // The group epilog runs unconditionally at the tail of each group, after
  // the peeled final-chunk block on the last group.
  // =========================================================
  uint chunk = 0;
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    // The last group runs one fewer chunk through this loop; its true final
    // chunk is handled by the peeled epilogue below instead. This is a
    // per-group branch (evaluated num_groups times total), not a per-chunk
    // one -- unrelated to the per-chunk conditional-epilog crash mode above.
    ...
    for (uint inner = 0; inner < chunks_this_group; ++inner, ++chunk) {
      ...
      //     reaches the true final chunk, see chunks_this_group above) ---
      {
    ...
    if (group_i + 1u == num_groups) {
      ...
      // step 1's `inner == 0u && group_i > 0u` branch above) applies here
      // too, using the same prefetched-by-the-previous-group values.
