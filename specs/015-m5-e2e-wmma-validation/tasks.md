...
- [ ] T012 [US1] E2E capture 1B/`4w` (depends on T007, T011): **3 repeated runs**, each 2048-token prefill / 1024-token decode, `ET_VK_EXECUTE_NODE_THRESHOLD=16`; record `prefill_tok_s`/`decode_tok_s` per run to `results/raw/`, then compute and record the mean + CoV per `research.md` Decision 5 — this establishes the shared 3-run procedure every subsequent e2e-capture task in this file follows. Single dispatch-confirm run already showed prefill jump from ~310 tok/s (broken texture-fallback PTE) to 553.8 tok/s (correct coopmat PTE) -- the formal 3-run mean is still needed before reporting
