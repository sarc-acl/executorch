...
layout(set = 0, binding = 1) buffer PRECISION restrict readonly t_mat1Buffer {
    f16vec4 t_mat1[];
};


layout(set = 0, binding = 2) buffer PRECISION restrict readonly t_weightBuffer {
    f16vec4 t_weight[];
};


layout(set = 0, binding = 3) buffer PRECISION restrict readonly t_biasBuffer {
    float16_t t_bias[];
};
...
layout(set = 0, binding = 4) uniform PRECISION restrict readonly mat1_sizes_UBO {
  ivec4 mat1_sizes;
};

layout(set = 0, binding = 5) uniform PRECISION restrict readonly out_sizes_UBO {
  ivec4 out_sizes;
};
