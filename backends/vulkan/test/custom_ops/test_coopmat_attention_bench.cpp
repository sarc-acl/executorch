...
//   QK*K^T : per head, out[s,c] = sum_d Q[s,d]*K[c,d]  -> M=seq, K=head_dim,
//   N=seq attn*V : per head, out[s,d] = sum_c P[s,c]*V[c,d]  -> M=seq, K=seq,
//   N=head_dim
...
//
// Each tile dispatches via test_etvk.test_mm with impl_selector
// "coopmat:<tile>"
// -> add_matmul_coopmat_node(tile_variant), which launches the per-variant
// workgroup size (= SG_GRID_X*SG_GRID_Y*SUBGROUP_SIZE). Correctness is
// validated on small aligned shapes against an fp32 reference before trusting
// the perf numbers (a wrong WG-size -> grid-stride staging overrun -> garbage
// output).
//
...
// One TestCase. impl is "tiled" (baseline column) or "coopmat:<suffix>" / a
// bare tile suffix that gets prefixed with "coopmat:". Empty suffix -> bare
// coopmat.
static TestCase make_case(
  ...
  ValueSpec mat1(
      {cfg.M, cfg.K}, dt, storage, utils::kWidthPacked, DataGenType::RANDOM);
  ValueSpec mat2(
      {cfg.K, cfg.N}, dt, storage, utils::kWidthPacked, DataGenType::RANDOM);
  ValueSpec output(
  ...
  // tiled accumulates in fp16 (error grows with K); coopmat accumulates in
  // fp32, bounded by fp16 input/output rounding only.
  if (is_tiled) {
...
int main() {
  ...
  std::cout
      << "fp16 attention-matmul TILE SWEEP: matmul_coopmat tile geometries "
