 ...
 * byte-identical at copy time, used by specs/008-8da4w-parameter-sweep to
 * build tile-shape/subgroup-size variants beyond the two the production
 * shader ships (see dq8ca_q4gsw_coopmat_sweep.yaml's shader_variants).
 * Kept separate rather than adding variants to the production yaml so
 * that ExecuTorch's shader codegen (keyed by filename stem) cannot ever
 * conflict with or affect what real models dispatch -- this workstream's
 * existing precedent for this pattern is coopmat_mm_ref.glsl, "the
 * test/custom_ops reference port" of a separate reference implementation.
 */
...
void main() {
  ...
  // --- A staging thread map: (m4, k4) slots, looped per-thread when the
  // count exceeds WG_SIZE (mirrors the INT4 B-staging pattern below) ---
  // (4 M-rows x 4 K-positions per slot; each slot expands to 4 slab-major
  // LDS uints.) test/custom_ops sweep fix: the original one-slot-per-thread
  // form silently left part of the tile's LDS staging buffer unwritten for
  // WG_TILE_M configs beyond the shipped 128 (e.g. 256), since
  // A_TOTAL_SLOTS then exceeds WG_SIZE -- see research.md Decision 4 (found
  // via config 8's empirically-confirmed correctness bug, root-caused by
  // deriving this exact formula). For A_SLOTS_PER_THREAD == 1 (every
  // previously-passing config), this loop is a no-op rewrite of the
  // original single-slot logic -- same bounds check, same indices.
  const uint K_BLOCKS_PER_CHUNK = WG_TILE_K >> 2u;
  const uint A_TOTAL_SLOTS = (WG_TILE_M >> 2u) * K_BLOCKS_PER_CHUNK;
  ...
  ivec4 temp_A[A_SLOTS_PER_THREAD];
#ifdef WEIGHT_INT4
  ...
  [[unroll]] for (uint asi = 0; asi < A_SLOTS_PER_THREAD; ++asi) {
    const uint a_slot = gl_LocalInvocationID.x + asi * WG_SIZE;
    if (a_slot < A_TOTAL_SLOTS) {
      const uint a_m_block = a_slot / K_BLOCKS_PER_CHUNK;
      const uint a_k_block = a_slot % K_BLOCKS_PER_CHUNK;
      const uint m4_global = (tile_m_start >> 2u) + a_m_block;
      temp_A[asi] = t_packed_int8_input[m4_global * nblocks_x_A + a_k_block];
    }
  }
#ifdef WEIGHT_INT4
      ...
      if (has_next) {
        ...
        [[unroll]] for (uint asi = 0; asi < A_SLOTS_PER_THREAD; ++asi) {
          ...
          if (a_slot < A_TOTAL_SLOTS) {
            ...
            temp_A[asi] = t_packed_int8_input[m4_global * nblocks_x_A + k4_global];
          }
        }
#ifdef WEIGHT_INT4
        [[unroll]] for (uint si = 0; si < B_SLOTS_PER_THREAD; ++si) {
