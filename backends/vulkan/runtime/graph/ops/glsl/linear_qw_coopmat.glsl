 ...
 * scale is applied during the B-tile store to shared memory: each int weight
 * is unpacked (nibble - 8), cast to fp16, and multiplied by its scale before
 * it lands in Bsh, keeping the K-loop a clean fp16 MMA.
 *
 * "dbuf4" double-buffered loop structure (store-first, prefetch-peeled):
 *   PROLOGUE: prefetch tile 0 into temp registers, then store it to
 *     shared-memory slice 0 (no barrier -- the main loop's first iteration
 *     barriers before reading slice 0).
 *   MAIN LOOP: one barrier per iteration, unconditional body --
 *     barrier -> prefetch(next chunk, into temp) -> MMA(current slice) ->
 *     store(temp, dequantized, into the other slice). The prefetch loads are
 *     in flight during the math and only consumed at the store stage.
 *   EPILOGUE: barrier, then MMA on the last chunk (loop peeled).
 * Ping-pong shared-memory slices make the overlap safe.
 *
 * IO_STORAGE selects between an SSBO-addressed coopMatLoad/coopMatStore
 * epilogue (buffer) and staging the result tile through shared memory before
 * an imageStore (texture3d) -- needed because coopMatStore itself can only
 * target a buffer.
 *
...
void main() {
  ...
  for (chunk = 0; chunk + 1u < num_chunks; ++chunk) {
    ...
    // coopMatLoad on the M51 Xclipse/AMD-PAL driver -- symptom is one stale
    // MMA_M-row band of A, all columns, ~2.5% of runs, no crash. The explicit
    // memoryBarrierShared() fence fixes it at no measured cost.
    memoryBarrierShared();
    barrier();
