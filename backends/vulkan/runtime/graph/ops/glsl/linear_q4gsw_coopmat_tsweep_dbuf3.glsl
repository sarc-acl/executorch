...
/*
 * TILE/SUBGROUP-SWEEP variant of linear_q4gsw_coopmat's dbuf3 loop structure
 * (specs/041-dbuf4-tile-sweep). Forked from linear_q4gsw_coopmat_tsweep.glsl
 * (dbuf1, the production winner) -- everything except the PROLOGUE/MAIN
 * LOOP/EPILOGUE block is identical: bindings, spec-constants, tile-geometry
 * templating, dequant_block, fp16 accumulator, bias/store epilogue. Only the
 * loop structure is swapped to dbuf3, recovered from the pre-retune
 * reference (linear_q4gsw_coopmat_dbuf3.glsl, see specs/041/reference/).
 * dbuf3 is dbuf2 ("store-first") with the last iteration peeled out so the
 * loop body has NO conditionals:
 *   prologue: prefetch chunk 0 -> temp (no barrier)
 *   per iter (0..num_chunks-2): store(cur) -> barrier -> MMA(cur) ->
 *     prefetch(next) [unconditional -- loop bound already excludes last]
 *   epilogue: store(last) -> barrier -> MMA(last) [no prefetch]
 *
 * Selected at dispatch via
 * ET_VK_Q4GSW_COOPMAT_VARIANT=tsweep_dbuf3_t<M>x<N>k<K>g<SGX><SGY>s<sub>
 * (QuantizedLinear.cpp), additive to the tsweep_t.../tsweep_dbuf2_t.../
...
void main() {
  ...
  // =========================================================
  // PROLOGUE (dbuf3): prefetch chunk 0 into temp registers only -- no
  // shared-memory write, no barrier here.
  // =========================================================
  {
  ...
  // =========================================================
  // MAIN LOOP (dbuf3) — last iteration peeled; NO conditionals in the body.
  // Iteration `chunk` (0..num_chunks-2) does:
  //   1. store        — temp (chunk, dequantized) into slice (chunk%2)
  //   2. barrier      — cur slice fully written
  //   3. MMA math     — on slice (chunk%2)
  //   4. prefetch     — chunk+1 from global into temp (unconditional)
  // =========================================================
  uint chunk;
  for (chunk = 0; chunk + 1u < num_chunks; ++chunk) {
    ...
    // --- prefetch chunk+1 -> temp (unconditional; loop bound excludes last) ---
    {
  ...
  // =========================================================
  // EPILOGUE: last chunk — store, barrier, MMA (no prefetch).
  // =========================================================
  {
    const uint cur_base_A = (chunk % 2u) * ASH_SLICE;
    const uint cur_base_B = (chunk % 2u) * BSH_SLICE;
