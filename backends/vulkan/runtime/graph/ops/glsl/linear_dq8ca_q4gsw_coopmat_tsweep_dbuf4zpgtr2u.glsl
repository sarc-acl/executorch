...
const uint A_SLAB_V4    = A_SLAB_U32 / 4u;   // still used by the thread map
shared uint Ash_int8[2u * ASH_SLICE_U32];    // uint[] -- as the BASELINE
...
void main() {
...
#endif
  {
    ...
    [[unroll]] for (uint s = 0; s < A_IV4_PER_THREAD; ++s) {
      const uint slot = gl_LocalInvocationID.x + s * WG_SIZE;
      if (slot < A_IV4_TOTAL) {
        const uint d = (a_lds_v4 + s * A_ROWS_PER_PASS) * 4u;
        Ash_int8[d + 0u] = uint(temp_A[s].x);
        Ash_int8[d + 1u] = uint(temp_A[s].y);
        Ash_int8[d + 2u] = uint(temp_A[s].z);
        Ash_int8[d + 3u] = uint(temp_A[s].w);
      }
    }
#ifdef WEIGHT_INT4
    [[unroll]] for (uint si = 0; si < B_SLOTS_PER_THREAD; ++si) {
  ...
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    for (uint inner = 0; inner < CHUNKS_PER_GROUP; ++inner, ++chunk) {
      ...
      const uint cur_a_u32 = (chunk % 2u) * ASH_SLICE_U32;
      const uint cur_b = (chunk % 2u) * BSH_SLICE_U32;
      const uint nxt_a_u32 = ((chunk + 1u) % 2u) * ASH_SLICE_U32;
      ...
      [[unroll]] for (uint k = 0; k < NUM_K_SLABS; ++k) {
        const uint slab_a_base_u32 = cur_a_u32 + k * A_SLAB_U32;
    ...
    [[unroll]] for (uint s = 0; s < A_IV4_PER_THREAD; ++s) {
      const uint slot = gl_LocalInvocationID.x + s * WG_SIZE;
      if (slot < A_IV4_TOTAL) {
        const uint d = nxt_a_u32 + (a_lds_v4 + s * A_ROWS_PER_PASS) * 4u;
        Ash_int8[d + 0u] = uint(temp_A[s].x);
        Ash_int8[d + 1u] = uint(temp_A[s].y);
        Ash_int8[d + 2u] = uint(temp_A[s].z);
        Ash_int8[d + 3u] = uint(temp_A[s].w);
