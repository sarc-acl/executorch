...
void main() {
  ...
  [[unroll]] for (uint s = 0; s < A_TILES_PER_SG; ++s) {
    ...
    if (t < NUM_A_TILES) {
      ...
      // Offset and stride are in ARRAY ELEMENTS (int = 4 int8), matching how
      // the MMA loop already addresses the shared uint arrays below.
      coopMatLoad(
          temp_A[s], t_packed_int8_input,
          (tile_m_start + tm * MMA_M) * nblocks_x_A + tk * (MMA_K >> 2u),
          nblocks_x_A,
      ...
      if (has_next) {
        ...
        [[unroll]] for (uint s = 0; s < A_TILES_PER_SG; ++s) {
          ...
          if (t < NUM_A_TILES) {
            ...
            coopMatLoad(
                temp_A[s], t_packed_int8_input,
                (tile_m_start + tm * MMA_M) * nblocks_x_A +
                    (chunkK_nxt >> 2u) + tk * (MMA_K >> 2u),
                nblocks_x_A,
                gl_CooperativeMatrixLayoutRowMajor);
          }
        }
