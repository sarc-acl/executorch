 ...
 * staging (coopMatLoad(global) -> coopmat<> -> coopMatStore(LDS)), PLUS tr3's
 * shared-A layout: scalar int8_t storage, row-major over the FULL chunk, with
 * A_ROW_PAD_I8 bytes of per-row padding. B staging, the quantization math and
 * every correctness safeguard are dbuf4zpgtr's, byte-for-byte unchanged. This is
 ...
 *             4 rows into shared A with 4 scalar stores.
 ...
 *             coopMatStore into shared A -- dbuf4tr's mapping,
...
// ===== tr3-style A shared layout: THIS FILE'S ONLY STRUCTURAL CHANGE =====
// dbuf4zpgtr stages A slab-major -- one MMA_K-wide slab per K step, row stride
// MMA_K/4 dwords, no padding, element type `uint` (4 int8 packed per element).
// tr3 instead keeps ONE row-major row per A row spanning the WHOLE chunk
// (WG_TILE_K wide), pads that row, and declares the array with the coopmat's
// own scalar element type so coopMatLoad/Store address it in elements.
//
// A_ROW_PAD_I8 pads the FULL-CHUNK row, NOT each MMA_K slab row -- the two have
// very different storage costs. Full-chunk padding costs WG_TILE_M*A_ROW_PAD_I8
// bytes per slice; padding every slab row would cost NUM_K_SLABS times that.
// tr3's own value is 16 bytes (== ELEMENTS_PER_VEC4 for an 8-bit type).
const uint A_ROW_PAD_I8  = ${A_ROW_PAD_I8}u;
const uint STRIDE_A_I8   = WG_TILE_K + A_ROW_PAD_I8;
const uint ASH_SLICE_I8  = WG_TILE_M * STRIDE_A_I8;

const uint BSH_SLICE_U32 = NUM_K_SLABS * B_SLAB_U32;

// Double-buffered MMA operand staging. A is scalar int8_t (tr3-style); B keeps
// dbuf4zpg's packed-uint column-major slab layout untouched at this stage.
shared int8_t Ash_i8[2u * ASH_SLICE_I8];
...
void main() {
  ...
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    for (uint inner = 0; inner < CHUNKS_PER_GROUP; ++inner, ++chunk) {
      ...
      const uint cur_a = (chunk % 2u) * ASH_SLICE_I8;
      const uint cur_b = (chunk % 2u) * BSH_SLICE_U32;
      const uint nxt_a = ((chunk + 1u) % 2u) * ASH_SLICE_I8;
      ...
      [[unroll]] for (uint k = 0; k < NUM_K_SLABS; ++k) {
        // row-major full-chunk A: the K step is a column offset within the
        // row, not a separate slab base.
        const uint k_col_a_i8 = k * MMA_K;
        ...
        [[unroll]] for (uint i = 0; i < MMAS_PER_SG_M; ++i) {
          ...
          coopMatLoad(
              matA[i], Ash_i8,
              cur_a + row_a * STRIDE_A_I8 + k_col_a_i8,
              STRIDE_A_I8,
              gl_CooperativeMatrixLayoutRowMajor);
        }
      ...
      if (has_next) {
        ...
        [[unroll]] for (uint s = 0; s < A_TILES_PER_SG; ++s) {
          ...
          if (t < NUM_A_TILES) {
            ...
            coopMatStore(
                temp_A[s], Ash_i8,
                nxt_a + (tm * MMA_M) * STRIDE_A_I8 + tk * MMA_K,
                STRIDE_A_I8,
                gl_CooperativeMatrixLayoutRowMajor);
          }
        }
