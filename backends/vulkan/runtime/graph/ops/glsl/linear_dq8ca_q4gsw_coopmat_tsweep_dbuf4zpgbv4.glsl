 ...
 * staging, PLUS "bv4": shared B retyped to uvec4 so each (K-slab, column)
 * column-slab is ONE 16-byte element written by ONE b128 store instead of 4
 * scalar ds_write_b32 spread across 4 threads. A staging, the quantization
 * math and every correctness safeguard are dbuf4zpgtr's, unchanged. This is
...
// ===== bv4: shared B is uvec4-typed, ONE element per (K-slab, column) =====
// dbuf4zpgtr stores B as `uint`: 4 uints per column-slab, written as 4 separate
// scalar ds_write_b32 spread over 4 threads. One column-slab is exactly
// MMA_K int8 = 16 bytes = one uvec4, and B_STRIDE_U32 is already 4 because zpg
// removed the anti-bank-conflict skew -- so those 4 uints are contiguous AND
// 16B-aligned. That alignment is what made a b128 store impossible on the old
// skewed layout (75% of columns were unaligned) and possible now.
// Each owning thread assembles all 4 widened uints and issues ONE store.
const uint BSH_SLICE_V4 = NUM_K_SLABS * WG_TILE_N;
shared uvec4 Bsh_v4[2u * BSH_SLICE_V4];
...
void main() {
...
#ifdef WEIGHT_INT4
  // bv4 ownership: thread t owns one whole (slab, col) column-slab when
  // t < BSH_SLICE_V4, and nothing otherwise. comp/par depend only on the
  // column, so a thread's 4 k4 fetches share them and differ only in k4 --
  // 4 consecutive k4 blocks of the SAME n8 texel column. Ownership is
  // subgroup-aligned (WG_TILE_N and BSH_SLICE_V4 are multiples of the subgroup
  // width for every shipped geometry), so a non-owning subgroup skips the whole
  // staging block on a wave-uniform branch rather than diverging.
  const bool b_owns   = gl_LocalInvocationID.x < BSH_SLICE_V4;
  const uint b_slab   = gl_LocalInvocationID.x / WG_TILE_N;
  const uint b_col    = gl_LocalInvocationID.x % WG_TILE_N;
  const uint b_rem    = b_col & 7u;
  const uint b_comp1  = b_rem & 3u;
  const uint b_par1   = b_rem >> 2u;
  const uint b_n8blk1 = (tile_n_start >> 3u) + (b_col >> 3u);
  const uint b_k4base = b_slab * (MMA_K >> 2u);
  const uint b_v4_off = b_slab * WG_TILE_N + b_col;
#endif
...
#ifdef WEIGHT_INT4
  ivec4 temp_B[MMA_K >> 2u];   // bv4: one per k4 of the owned column-slab
  float temp_wsc;
#else
  ivec4 temp_B;
#endif
...
#ifdef WEIGHT_INT4
  if (b_owns) {
    [[unroll]] for (uint kk = 0; kk < (MMA_K >> 2u); ++kk) {
#ifdef WEIGHT_BUFFER
      temp_B[kk] = t_packed_weight[(b_n8blk1 * nblocks_x_A) + b_k4base + kk];
#else
      temp_B[kk] = texelFetch(t_packed_weight, ivec2(b_k4base + kk, b_n8blk1), 0);
#endif
    }
  }
#else
...
#ifdef WEIGHT_INT4
    if (b_owns) {
      // ONE b128 store replaces the 4 scalar ds_write_b32 this column-slab
      // previously took (spread across 4 threads).
      Bsh_v4[ b_v4_off] = uvec4(
          widen_nibbles(uint(temp_B[0][b_comp1]), b_par1),
          widen_nibbles(uint(temp_B[1][b_comp1]), b_par1),
          widen_nibbles(uint(temp_B[2][b_comp1]), b_par1),
          widen_nibbles(uint(temp_B[3][b_comp1]), b_par1));
    }
#else
    if (b_active) {
      ...
      [[unroll]] for (uint n_in_blk = 0u; n_in_blk < 4u; ++n_in_blk) {
        // bv4: identical address, expressed as (uvec4 element, component).
        // This INT8 path writes 4 DIFFERENT columns for one k4, so it cannot
        // use a wide store -- it keeps scalar component writes.
        Bsh_v4[slab_idx * WG_TILE_N + (n_col_base + n_in_blk)][k4_in_slab] =
  ...
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    for (uint inner = 0; inner < CHUNKS_PER_GROUP; ++inner, ++chunk) {
      ...
      const uint cur_b_v4 = (chunk % 2u) * BSH_SLICE_V4;
      const uint nxt_a = ((chunk + 1u) % 2u) * ASH_SLICE_U32;
      const uint nxt_b = ((chunk + 1u) % 2u) * BSH_SLICE_V4;
      ...
      if (has_next) {
        ...
        if (b_owns) {
          [[unroll]] for (uint kk = 0; kk < (MMA_K >> 2u); ++kk) {
            const uint k4_blk = (chunkK_nxt >> 2u) + b_k4base + kk;
#ifdef WEIGHT_BUFFER
            temp_B[kk] = t_packed_weight[(b_n8blk1 * nblocks_x_A) + k4_blk];
#else
            temp_B[kk] = texelFetch(t_packed_weight, ivec2(k4_blk, b_n8blk1), 0);
#endif
          }
        }
    ...
    if (b_owns) {
      ...
      Bsh_v4[nxt_b + b_v4_off] = uvec4(
        ...
        if (b_active) {
          ...
          const uint n_col_base = b_n_uint_col * 4u;
          [[unroll]] for (uint n_in_blk = 0u; n_in_blk < 4u; ++n_in_blk) {
            Bsh_v4[nxt_b + slab_idx * WG_TILE_N + (n_col_base + n_in_blk)][k4_in_slab] =
                uint(temp_B[n_in_blk]);
          }
        }
#endif
