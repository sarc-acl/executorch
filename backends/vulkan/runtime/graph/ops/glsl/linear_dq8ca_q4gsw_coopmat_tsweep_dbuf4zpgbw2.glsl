...
void main() {
...
#ifdef WEIGHT_INT4
  // ===== bw2 DIAGNOSTIC: 2 consecutive uints per thread, ALL 256 threads
  // active (no idling). Discriminates "4-wide ownership + idle threads" from
  // "consecutive-write ownership per se" as the cause of the num_groups==2
  // failure seen in bw/bwr.
  const uint B_PAIR   = 2u;
  const uint b_lin0   = gl_LocalInvocationID.x * B_PAIR;   // first uint index
  const uint b_slab   = b_lin0 / B_SLAB_U32;
  const uint b_inslab = b_lin0 % B_SLAB_U32;
  const uint b_col    = b_inslab / B_STRIDE_U32;
  const uint b_k4lo   = b_inslab % B_STRIDE_U32;
  ...
  const uint b_k4base = b_slab * (MMA_K >> 2u) + b_k4lo;
  const uint b_lds_base = b_lin0;
#endif
...
#ifdef WEIGHT_INT4
  ivec4 temp_B[2u];   // bw2: one per k4 of the owned pair
  float temp_wsc;
#else
  ivec4 temp_B;
#endif
