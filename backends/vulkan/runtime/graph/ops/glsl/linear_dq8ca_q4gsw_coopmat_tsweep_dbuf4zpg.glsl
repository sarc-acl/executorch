 ...
 *
 * INTERVENTION F variant: all loop-invariant staging index arithmetic is
 * hoisted out of the group loop (see "INTERVENTION F" below).
 ...
 *
 * Selected via
 * ET_VK_DQ8CA_COOPMAT_VARIANT=tsweep_dbuf4zpn_t<M>x<N>k<K>g<SGX><SGY>s<32|64>.
 *
...
// No skew + coalesced write, ported from the production dbuf4.glsl fix
// (dq8ca-dequant-unpack-ablation Addenda 6/9): stride=4 beats stride=5 on
// this hardware, and writing via the inverted contiguous-index mapping
// (see the F-hoist block below) beats the natural strided-scatter mapping.
...
void main() {
...
#ifdef A_ALWAYS_ACTIVE
  // A_ACTIVE_THREADS == WG_SIZE for this variant's tile, so every thread stages
  // A and the guard is unconditionally true.
  const bool a_active = true;
#else
  ...
  // ===== INTERVENTION F: hoist loop-invariant staging index math =====
  // Every term below depends only on gl_LocalInvocationID.x, si, tile_n_start
  // and tile_m_start -- none of them on `chunk` or `group_i`. In the baseline
  // this same 10-line decomposition is recomputed at THREE sites (prologue
  // store, in-loop prefetch, in-loop store) on every chunk iteration. The
  // ablation ladder attributes -16.8% to the B staging block and shows that
  // most of it is index arithmetic, not memory traffic: deleting B staging
  // removed 45 of the loop's 96 address instructions and 56 of its 64
  // compare/select instructions, but only 24 memory ops.
  //
  // Only the k4 texel row varies with the chunk, and it varies by a constant
  // stride, so the loop carries an increment instead of a recomputation.
#ifdef WEIGHT_INT4
  ...
  // Coalesced-write inversion (ported from production dbuf4.glsl's
  // bcoal_index, dq8ca-dequant-unpack-ablation Addendum 9): start from the
  // CONTIGUOUS per-thread LDS index `a` this thread will write to, and derive
  // which global-fetch element it needs -- instead of deriving the LDS
  // address from the natural fetch grouping (which is what produced the
  // B_STRIDE_U32-strided, non-coalesced writes this fix replaces).
...
#endif
  // A staging: same argument. base_row/slab/k_uint are all invariant.
  const uint a_lds_off0 =
      (a_k_block / (MMA_K >> 2u)) * A_SLAB_U32
      + (a_m_block * 4u) * A_STRIDE_U32
      + (a_k_block % (MMA_K >> 2u));
  const uint a_glb_row = ((tile_m_start >> 2u) + a_m_block) * nblocks_x_A;
...
#endif
  {
...
#endif
    {
      [[unroll]] for (uint m4i = 0; m4i < 4u; ++m4i) {
        Ash_int8[a_lds_off0 + m4i * A_STRIDE_U32] = uint(temp_A[m4i]);
      }
    }
#ifdef WEIGHT_INT4
    [[unroll]] for (uint si = 0; si < B_SLOTS_PER_THREAD; ++si) {
