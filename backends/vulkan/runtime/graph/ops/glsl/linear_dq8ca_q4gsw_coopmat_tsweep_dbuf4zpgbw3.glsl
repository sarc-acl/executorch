 ...
 * staging, PLUS "bw3": shared B retyped to uvec2 with dbuf4zpgbw2's
 * all-threads-active ownership, so each thread issues ONE ds_write_b64 for its
 * 2-uint pair. A staging, the quantization math and every safeguard are
 * dbuf4zpgtr's, unchanged. This is
...
// ===== bw3: shared B retyped to uvec2 =====
// The ONLY combination that gets both a genuinely wide LDS store and the
// safe ownership. A uvec4 element would be a whole column-slab, forcing
// 4-wide-per-thread ownership with half the workgroup idle -- which is exactly
// the pattern that fails the num_groups==2 shapes (see dbuf4zpgbw/bwr/bv4).
// A uvec2 element is 2 uints, so BSH_SLICE_V2 == WG_SIZE at the shipped
// geometry: one element per thread, ALL 256 threads active (dbuf4zpgbw2's
// proven-correct ownership), one ds_write_b64 each.
// Packing ratio for the compute-side coopMatLoad is 8x (int8 coopmat from
// uvec2[]); dbuf4zpgbw2 showed adjacent scalar writes do NOT get merged by the
// compiler, so retyping is the only way to actually emit a wide store.
const uint B_STRIDE_V2   = B_STRIDE_U32 / 2u;   // uvec2 per column
const uint B_SLAB_V2     = B_SLAB_U32 / 2u;
const uint BSH_SLICE_V2  = BSH_SLICE_U32 / 2u;
const uint V2_PER_THREAD = BSH_SLICE_V2 / WG_SIZE;
shared uvec2 Bsh_v2[2u * BSH_SLICE_V2];
...
void main() {
...
#ifdef WEIGHT_INT4
  // bw3 ownership == dbuf4zpgbw2's (proven correct), one uvec2 element each.
  uint b_v2off[V2_PER_THREAD];  // uvec2 element index within a slice
  uint b_comp1[V2_PER_THREAD];
  uint b_par1[V2_PER_THREAD];
  uint b_n8b[V2_PER_THREAD];
  uint b_k4b[V2_PER_THREAD];    // k4 of the FIRST uint of the pair
  [[unroll]] for (uint si = 0; si < V2_PER_THREAD; ++si) {
    const uint e        = gl_LocalInvocationID.x + si * WG_SIZE;  // uvec2 index
    const uint u0       = e * 2u;                                 // first uint
    const uint slab_idx = u0 / B_SLAB_U32;
    const uint inslab   = u0 % B_SLAB_U32;
    const uint n_col    = inslab / B_STRIDE_U32;
    const uint k4_lo    = inslab % B_STRIDE_U32;
    const uint rem      = n_col & 7u;
    b_v2off[si] = e;
    b_comp1[si] = rem & 3u;
    b_par1[si]  = rem >> 2u;
    b_n8b[si]   = (tile_n_start >> 3u) + (n_col >> 3u);
    b_k4b[si]   = slab_idx * (MMA_K >> 2u) + k4_lo;
  }
#endif
...
#ifdef WEIGHT_INT4
  ivec4 temp_B[V2_PER_THREAD * 2u];   // bw3: 2 packed fetches per uvec2
  float temp_wsc;
#else
  ivec4 temp_B;
#endif
...
#ifdef WEIGHT_BUFFER
      temp_B[si * 2u + h] = t_packed_weight[(b_n8b[si] * nblocks_x_A) + b_k4b[si] + h];
#else
      temp_B[si * 2u + h] = texelFetch(t_packed_weight, ivec2(b_k4b[si] + h, b_n8b[si]), 0);
#endif
    }
  }
#else
...
#ifdef WEIGHT_INT4
    [[unroll]] for (uint si = 0; si < V2_PER_THREAD; ++si) {
      // ONE ds_write_b64 per uvec2 element.
      Bsh_v2[ b_v2off[si]] = uvec2(
          widen_nibbles(uint(temp_B[si * 2u + 0u][b_comp1[si]]), b_par1[si]),
          widen_nibbles(uint(temp_B[si * 2u + 1u][b_comp1[si]]), b_par1[si]));
    }
#else
    if (b_active) {
      ...
      [[unroll]] for (uint n_in_blk = 0u; n_in_blk < 4u; ++n_in_blk) {
        // bw3: same address as (uvec2 element, component). This INT8 path
        // writes 4 different columns for one k4, so no wide store applies.
        {
          const uint u = slab_idx * B_SLAB_U32 + (n_col_base + n_in_blk) * B_STRIDE_U32 + k4_in_slab;
          Bsh_v2[u / 2u][u % 2u] = uint(temp_B[n_in_blk]);
        }
      }
    }
#endif
  ...
  for (uint group_i = 0; group_i < num_groups; ++group_i) {
    for (uint inner = 0; inner < CHUNKS_PER_GROUP; ++inner, ++chunk) {
      ...
      const uint cur_b_v2 = (chunk % 2u) * BSH_SLICE_V2;
      const uint nxt_a = ((chunk + 1u) % 2u) * ASH_SLICE_U32;
      const uint nxt_b_v2 = ((chunk + 1u) % 2u) * BSH_SLICE_V2;
      ...
      if (has_next) {
        ...
        [[unroll]] for (uint si = 0; si < V2_PER_THREAD; ++si) {
          [[unroll]] for (uint h = 0; h < 2u; ++h) {
            const uint k4_blk = (chunkK_nxt >> 2u) + b_k4b[si] + h;
#ifdef WEIGHT_BUFFER
            temp_B[si * 2u + h] = t_packed_weight[(b_n8b[si] * nblocks_x_A) + k4_blk];
#else
            temp_B[si * 2u + h] = texelFetch(t_packed_weight, ivec2(k4_blk, b_n8b[si]), 0);
#endif
          }
        }
      ...
      [[unroll]] for (uint k = 0; k < NUM_K_SLABS; ++k) {
        ...
        const uint slab_b_base_v2 = cur_b_v2 + k * B_SLAB_V2;
        ...
        [[unroll]] for (uint j = 0; j < MMAS_PER_SG_N; ++j) {
          ...
          coopMatLoad(
              matB, Bsh_v2,
              slab_b_base_v2 + col_b * B_STRIDE_V2,
              B_STRIDE_V2,
    ...
    [[unroll]] for (uint si = 0; si < V2_PER_THREAD; ++si) {
      // ONE ds_write_b64 per uvec2 element.
      Bsh_v2[nxt_b_v2 + b_v2off[si]] = uvec2(
          widen_nibbles(uint(temp_B[si * 2u + 0u][b_comp1[si]]), b_par1[si]),
          widen_nibbles(uint(temp_B[si * 2u + 1u][b_comp1[si]]), b_par1[si]));
    }
        if (group_crossing && gl_LocalInvocationID.x < WG_TILE_N) {
        ...
        if (b_active) {
          ...
          [[unroll]] for (uint n_in_blk = 0u; n_in_blk < 4u; ++n_in_blk) {
            {
              const uint u = slab_idx * B_SLAB_U32 + (n_col_base + n_in_blk) * B_STRIDE_U32 + k4_in_slab;
              Bsh_v2[nxt_b_v2 + u / 2u][u % 2u] = uint(temp_B[n_in_blk]);
            }
          }
        }
#endif
