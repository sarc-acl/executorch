 ...
 * staging, PLUS "bw": B staging ownership moved from (block,col) slots spread
 * over 4 threads to ONE thread per (K-slab, column), writing its 4 uints at
 * consecutive LDS indices so they are merge-eligible. Shared-array types, the
 * compute-side loads, the quantization math and every safeguard are
...
void main() {
...
#ifdef WEIGHT_INT4
  // ===== bw: SOURCE-side change only -- one thread owns a whole (slab, col)
  // column-slab and writes its 4 uints at CONSECUTIVE indices, so the four
  // ds_write_b32 are merge-eligible. The shared array stays `uint` (the proven
  // 4x packing ratio) and the compute-side coopMatLoad is byte-for-byte
  // unchanged -- deliberately NOT the uvec4 retype, which fails correctness on
  // this driver (16x packing; see the 8x coopMatStore precedent).
  // comp/par depend only on the column, so a thread's 4 fetches share them and
  // differ only in k4: 4 consecutive k4 blocks of the same n8 texel column.
  const uint B_OWNERS  = NUM_K_SLABS * WG_TILE_N;
  const bool b_owns    = gl_LocalInvocationID.x < B_OWNERS;
  ...
  // same address the destination-oriented map produced, just all 4 in one thread
  const uint b_lds_base = b_slab * B_SLAB_U32 + b_col * B_STRIDE_U32;
#endif
...
#ifdef WEIGHT_INT4
  ivec4 temp_B[MMA_K >> 2u];   // bw: one per k4 of the owned column-slab
  float temp_wsc;
#else
  ivec4 temp_B;
#endif
...
#ifdef WEIGHT_INT4
    if (b_owns) {
      // 4 CONSECUTIVE uints from one thread -> merge-eligible into a wider
      // store, versus 4 scalar stores from 4 different threads before.
      [[unroll]] for (uint q = 0; q < (MMA_K >> 2u); ++q) {
        const uint kk = (MMA_K >> 2u) - 1u - q;   // DIAGNOSTIC: reverse order
        Bsh_int8[b_lds_base + kk] =
            widen_nibbles(uint(temp_B[kk][b_comp1]), b_par1);
      }
    }
#else
    if (b_active) {
