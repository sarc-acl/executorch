...
/*
 * specs/023-8da4w-int8-dbuf-sweep: byte-identical loop-structure copy of the
 * shipped linear_dq8ca_qw_coopmat.glsl (already dbuf4, "store-first" -- see
 * below), built as its own op (linear_dq8ca_q4gsw_coopmat_dbuf4, selected via
 * ET_VK_DQ8CA_COOPMAT_VARIANT=dbuf4) so the in-sweep dbuf1/2/3 comparisons
 * have an apples-to-apples dbuf4 measurement under this same harness, rather
 * than reusing a number captured a different way. The production shader and
 * its default dispatch are untouched.
 *
