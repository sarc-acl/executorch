...
static std::string q4gsw_default_variant(ComputeGraph* graph) {
  ...
  // Qualcomm Adreno 840: fp16 MMA only at M=64 (64x{64,32,16}x16), f16
  // accumulate, subgroup fixed at 64. 64x64x32 WG tile, 2x1 subgroups.
  if (adapter->supports_cooperative_matrix_shape(
          64,
          32,
          16,
          ...
          VK_COMPONENT_TYPE_FLOAT16_KHR,
          VK_COMPONENT_TYPE_FLOAT16_KHR)) {
    return "tsweep_dbuf4_t64x64k32g21s64m64x32x16";
  }
...
// Capability-driven default dq8ca variant (int8 MMA -> int32).
static std::string dq8ca_default_variant(ComputeGraph* graph) {
  const auto* adapter = graph->context()->adapter_ptr();
  if (adapter->supports_cooperative_matrix_shape(
          16,
          16,
          ...
          VK_COMPONENT_TYPE_SINT32_KHR)) {
    return "tsweep_dbuf4zpgtr_t128x64k32g42s32"; // AMD RDNA
  }
  // Qualcomm Adreno 840: int8 MMA 64x{64,32,16}x32, subgroup 64.
  if (adapter->supports_cooperative_matrix_shape(
          64,
          64,
          32,
          ...
          VK_COMPONENT_TYPE_SINT32_KHR,
          VK_COMPONENT_TYPE_SINT32_KHR)) {
    return "tsweep_dbuf4zpgtr_t128x64k32g12s64m64x64x32";
  }
  return "tsweep_dbuf4zpgtr_t128x64k32g42s32"; // gate rejects on other devices
}
...
static const std::string& dq8ca_coopmat_variant(ComputeGraph* graph) {
  ...
  static const std::string variant = [graph] {
    ...
    if (!env) {
      return dq8ca_default_variant(graph);
    }
    const std::string v(env);
