...
BENCH = REPO / "cmake-out-nt/backends/vulkan/test/custom_ops/test_coopmat_linear_bench"
RUNNER = REPO / "cmake-out-nt/examples/models/llama/llama_main"

CKPT = Path("/mnt/linux-share/models/llama-3.2-1b")

# The coopmat path is gated to wave64 AMD unless this is set; without it every
# candidate silently falls back to tiled and the whole sweep ranks one kernel.
BASE_ENV = {"ET_VK_COOPMAT_ANY_DEVICE": "1"}
DEFAULTS = {
    "q4gsw": {
        "pte": CKPT / "exported/llama3_2-1b_vulkan_4w.pte",
        "env_var": "ET_VK_Q4GSW_COOPMAT_VARIANT",
        "kernel_base": "linear_q4gsw_coopmat",
    },
    "dq8ca": {
        "pte": CKPT / "exported/llama3_2-1b_vulkan_8da4w.pte",
        "env_var": "ET_VK_DQ8CA_COOPMAT_VARIANT",
        "kernel_base": "linear_dq8ca_q4gsw_coopmat",
    },
}
TOKENIZER = CKPT / "original/tokenizer.model"
PROMPT = Path.home() / "vk-bench/p2048_exact.txt"
...
class Session:
    def __init__(
        ...
        # Candidates travel as canonical "tsweep_t..." tokens; the runtime env
        # vars and shader NAMEs carry the family prefix ("tsweep_dbuf4_t...").
        # Feeding the canonical form straight through makes every candidate
        # look like an alignment fallback.
        self._rt = lambda tok: yv.runtime_token(shader, tok)
    ...
    def _run_llama(self, token=None):
        ...
        if token:
            env[self.env_var] = self._rt(token)
        proc = subprocess.run(
            [
                str(RUNNER),
                "--model_path",
    ...
    def gate(self, token):
        ...
        env = {
            **os.environ,
            **BASE_ENV,
            self.env_var: self._rt(token),
            "COOPMAT_BENCH_CORRECTNESS_ONLY": "1",
        }
        proc = subprocess.run(
        ...
        if "Could not find ShaderInfo" in out:
            ...
            dispatched = len(re.findall(rf"{self.kernel_base}_{self._rt(token)}", out))
            if fails:
