...
MMA = 16  # MMA_M and MMA_N, fixed across every coopmat yaml
FP16_PER_VEC4 = 8

# MMA_K is per-shader and per-device: it must be a shape the device enumerates
# in VkCooperativeMatrixPropertiesKHR. AMD-RDNA exposes int8 at 16x16x16, NVIDIA
# only at 16x16x32, so dq8ca's K differs by device while q4gsw's fp16 MMA is
# 16x16x16 everywhere the shaders run.
DEFAULT_MMA_K = {"q4gsw": 16, "dq8ca": 16}
...
def lds_bytes_q4gsw(m, n, k, gy=0, mma_k=16):
    """linear_q4gsw_coopmat_tsweep_dbuf4.glsl: fp16-staged A/B with a +1-vec4
    skew, double-buffered, plus the bias and Csh epilogue arrays.

    022's formula covered Ash+Bsh only. Csh is declared unconditionally in the
    shader, so it consumes LDS on the buffer path too and belongs in the
    budget."""
    del mma_k  # q4gsw's staging is vec4-based; MMA_K does not change its LDS
    a_stride_vec4 = (k + FP16_PER_VEC4) // FP16_PER_VEC4
    b_stride_vec4 = (n + FP16_PER_VEC4) // FP16_PER_VEC4
    staging = 2 * (m * a_stride_vec4 + k * b_stride_vec4) * 16
    csh = gy * MMA * n * 2
    bias = n * 2
    return staging + csh + bias


def lds_bytes_dq8ca(m, n, k, gy=0, mma_k=16):
    """linear_dq8ca_q4gsw_coopmat_tsweep_dbuf4zpgtr.glsl.

    Differs from 025's formula in two ways: dbuf4zpg removed B's +1 skew in
    favour of a coalesced write, and the -tr epilogue stages results through
    Csh_out. Both A and B slabs scale with MMA_K while NUM_K_SLABS shrinks,
    so the staging total is MMA_K-invariant -- but the rest is not."""
    num_k_slabs = k // mma_k
    a_slab_u32 = (m * mma_k) // 4
    ash_bytes = 2 * num_k_slabs * a_slab_u32 * 4
    b_stride_u32 = mma_k // 4  # no skew (dbuf4zpg's coalesced-write B fix)
    bsh_bytes = 2 * num_k_slabs * n * b_stride_u32 * 4
    # izp_sh, ifs_sh (per tile row); wsc_sh (ping-ponged), wcorr_sh, bias_sh.
    broadcast_bytes = m * 4 + m * 4 + 2 * n * 4 + n * 4 + n * 4
    csh_bytes = gy * MMA * n * 2
    return ash_bytes + bsh_bytes + broadcast_bytes + csh_bytes


def derive(shader, m, n, k, gx, gy, sub, group_size, limits, mma_k=None):  # noqa: C901
    ...
    if k % mma_k != 0:
        reasons.append(f"wg_tile_k={k} not divisible by MMA_K={mma_k}")
    ...
    if shader == "q4gsw":
        ...
        lds = lds_bytes_q4gsw(m, n, k, gy, mma_k)
    else:
        ...
        lds = lds_bytes_dq8ca(m, n, k, gy, mma_k)
...
def derive_token(shader, tok, group_size, limits, mma_k=None):
    return derive(
        shader,
...
def enumerate_legal(shader, limits, group_size, mma_k=None):
    out = []
                ...
                for gx in SG_GRID_CHOICES:
                    for gy in SG_GRID_CHOICES:
                        for sub in limits.subgroup_sizes:
                            c = derive(
                                shader, m, n, k, gx, gy, sub, group_size, limits, mma_k
                            )
                            if c["valid"]:
                                out.append(c)
    return out
