...
def main():
    ...
    ap.add_argument("--reps",  type=int, default=3,
                    help="Measurement subprocess count (default 3).")
    ap.add_argument("--iters", type=int, default=8,
                    help="Iterations per measurement subprocess (default 8).")
    ap.add_argument("--num_executions", type=int, default=None,
    ...
    if args.num_executions is not None:
        ...
        result = bench_steady_state(pte_path, input_path, etdump_path, mem_log,
                                     n_reps=args.reps, n_iters_per_rep=args.iters)
        if result is None:
            sys.exit(3)
        probe = None
